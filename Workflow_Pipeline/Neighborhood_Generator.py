...
def contigend_visualization(contig_array,end_direction,genome,reverse_term,drug,rgi_gene):
    ...
    if end_direction=="downward":
        b=GraphicFeature(start=contig_array["GeneEnd"].iloc[-1]+1500, end=contig_array["GeneEnd"].iloc[-1]+1500, strand=+1,color="#0A090A",label="Contig_Ends")
        Features.append(b) 
        temp_array_totrack_length.append((contig_array["GeneEnd"].iloc[-1]+1500,contig_array["GeneEnd"].iloc[-1]+1500))
  ...
  if(len(m)!=0):
    if(len(n)<g ):
        ...
        if number_of_genes==10:
            contigend_visualization(e,"downward",genome,rec["Orientation"][j],drug,rec["Best_Hit_ARO"][j])
        ...
        print(genome)
        if number_of_genes==10:
            contigend_visualization(e,"upward",genome,rec["Orientation"][j],drug,rec["Best_Hit_ARO"][j])
        neighbor_genes=[]          
...
### Finding neighbours(5 upstream and 5 dowmstream genes for each card gene)
def find_neighbor_onefile(rec,uname,data,number_of_genes,drug,genome):
  neighbor_genes=[]
  rec.reset_index(drop=True, inplace=True)
  ...
  for j in range(len(rec['Start'])):
        ...
        contig_visualization_onefile(e,genome,drug,rec['Best_Hit_ARO'][j])         
            
if cluster_key==False:
    for i,j in Dict_singlegene_instances.items():
        for k in j.keys():
            find_neighbor_onefile(j[k],uniquedict[k],datasetdict[k],10,i,k)
